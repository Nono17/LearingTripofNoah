...
public class GridImageView<T> extends ViewGroup {
    ...
    private int mGap=20; //间隙,默认为5px
    private int mColumnCount=3; // 列数
    private int mGridSize; //每个条目的大小
    private double mGridLengthWidthRadio=0.7;//单个格子长宽比例
    ...
    private void measureHorizontal(int widthMeasureSpec, int heightMeasureSpec){
        ...
        height = (int) (mGridSize * mGridLengthWidthRadio  + getPaddingTop() + getPaddingBottom());//计算出高度
    ...
    private void measureVertical(int widthMeasureSpec, int heightMeasureSpec){
        ...
        height = (int)(mGridSize * mGridLengthWidthRadio * mRowCount + mGap * (mRowCount - 1) + getPaddingTop() + getPaddingBottom());//计算出高度
        setMeasuredDimension(width, height);
    }
    ...
    private void layoutVertical(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childrenCount; i++) {
            ...
            int top = ((int) (mGridSize * mGridLengthWidthRadio) + mGap) * rowNum + getPaddingTop();
            int right = left + mGridSize;
            int bottom = top + (int) (mGridSize * mGridLengthWidthRadio);
            childrenView.layout(left, top, right, bottom);
        }
